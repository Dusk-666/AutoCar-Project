...
static void Encoder_UpdateChannel(EncoderChannel *channel, float filter_alpha)
{
    ...
    signed_delta = (int16_t)((int16_t)(current_counter - channel->last_counter) * channel->forward_sign);
    channel->last_counter = current_counter;
    channel->filtered_speed += filter_alpha * ((float)signed_delta - channel->filtered_speed);
